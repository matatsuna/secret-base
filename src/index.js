...
const createFlower = ((x = 0, y = 0, dy = -3, scale = 1, color ="#f08080") => {
    ...
    flowerTranslate.transform("translate(" + x + "," + y + ")")
    flowerScale.transform("scale(" + scale + "," + scale + ")");

    setInterval((() => {
        flower.attr({
            transform: flower.matrix.rotate(2, bbox.cx, bbox.cy)
        })
        flowerTranslate.attr({
            transform: flowerTranslate.matrix.translate(0, dy)
        })
    }), 50);
    return flower;
});
